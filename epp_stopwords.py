...
'''Extension file. Everything is the same except the count_words() function,
but I duplicated the file out of an abundance of caution and in the interest of avoiding 
any issues running the original code.'''
...
## function with stopwords extension
def count_words(email_path='data/enron', stopwords = False):
    ...
    '''
    with open('data/stopwords', 'r', encoding = 'latin-1') as stopwords:
        stopwords_list = tokenize_words(stopwords.read())
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron'):
    ...
    # read file as string and update dictionary
    features = np.zeros((num_emails, len(top_words)))
    ...
    for f in list_files:
        with open(f, 'r', encoding='latin-1') as file:
            if not os.path.basename(f).startswith('.'):
                ...
                features[i] = np.array(list(word_freq.values()))
                i+=1
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    test_idx = inds[:split_pt]
    train_idx = inds[split_pt:]
